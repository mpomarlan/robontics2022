...
class RuleTemplate:
    ...
    def instantiate(self, bdg):
        ...
        return (tuple(antecedent), self._operator, tuple(consequent), (self._id, bdgTp, tuple(self.dominates)))
...
def netDominates(a, b):
    provA, bdgA, subsA = a
    provB, bdgB, _ = b
    bdgA = {k:v for k,v in bdgA}
    bdgB = {k:v for k,v in bdgB}
    if provB in subsA:
        aK = set(bdgA.keys())
        bK = set(bdgB.keys())
        if all([(bdgA[k] == bdgB[k]) for k in aK.intersection(bK)]):
            return True
    return False
...
def loadDFLRules(inFile):
    ...
    return [retq[k] for k in retq.keys()]
def loadDFLFacts(inFile, facts):
    lines = [y for y in [x.strip() for x in open(inFile).read().splitlines()] if y and ('#'!=y[0])]
    for l in lines:
